...
def eval_FP(det_results, 
            ...
            use_legacy_coordinate=False,
            use_group_of=False):
    """ Evaluate Actual False Postives of a dataset for each class.
        Author: Yuxuan Hu
        Date: 2023/12/23
    """
    assert len(det_results) == len(annotations)
    ...
    all_dets = [np.concatenate(img_res, axis=0) for img_res in det_results]
    all_dets_labels = []
    for img_res in det_results:
        for i in range(num_classes):
            all_dets_labels += [i] * img_res[i].shape[0]
    all_gts = [ann['bboxes'] for ann in annotations]
    all_gts_ignore = [np.empty((0, 4), dtype=np.float32) for _ in annotations] # No ignore
    ...
    if num_imgs > 1:
        # compute tp and fp for each image with multiple processes
        args = []
        assert not use_group_of
        if ioa_thr is not None:
            args.append([ioa_thr for _ in range(num_imgs)])

        tpfp = pool.starmap(
            tpfp_fn,
            zip(all_dets, all_gts, all_gts_ignore,
                [iou_thr for _ in range(num_imgs)],
                [area_ranges for _ in range(num_imgs)],
                [use_legacy_coordinate for _ in range(num_imgs)], *args))
    else:
    ...
    eval_results = []
    for i in range(num_classes):
        eval_results.append(list(np.array(all_dets_labels)[tuple([fp[0]==1])]).count(i))
    return eval_results
...
def print_map_summary(mean_ap,
    ...
    header = ['class', 'gts', 'dets', 'recall', 'precision', 'TP', 'FP','ap']
    for i in range(num_scales):
        if scale_ranges is not None:
        ...
        for j in range(num_classes):
            row_data = [
                ...
                f'{recalls[i, j]:.4f}', f'{precisions[i, j]:.4f}', results[j]['tp'], results[j]['fp'], f'{aps[i, j]:.4f}'
            ]

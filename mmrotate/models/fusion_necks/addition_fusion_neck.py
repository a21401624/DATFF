...
class AdditionFusionNeck(BaseModule):
    ...
    def __init__(self,
        ...
        super(AdditionFusionNeck, self).__init__(init_cfg)
    ...
    def forward(self, inputs1, inputs2, **kwargs):
        ...
        if self.no_conv:
            outs = [
                (inputs1[i] + inputs2[i]) for i in range(self.num_outs)
            ]
        else:
            outs = [
                self.fusion_convs[i]((inputs1[i] + inputs2[i])) for i in range(self.num_outs)
            ]
        if self.version == '2.0':
            outs = [x * 0.5 for x in outs]
        return tuple(outs)